...
import { Download, RotateCcw, ZoomIn, ZoomOut, Sun, Moon, AlertTriangle } from "lucide-react";
...
export default function ThreeDViewer({
  ...
  const isComplete = phase === "complete" && modelUrl;
  const isGenerating = phase === "generating";

  return (
    <div className="relative w-full h-full rounded-xl overflow-hidden glass-card glow-indigo">
      {/* Header */}
      <div
        className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-4 py-3"
        style={{ background: "linear-gradient(to bottom, rgba(8,11,20,0.9) 0%, transparent 100%)" }}
      >
        <div className="flex items-center gap-2">
          <div className={`status-dot ${isGenerating ? "processing" : isComplete ? "active" : ""}`}
            style={{ background: isComplete ? "#10b981" : isGenerating ? "#f59e0b" : "#475569" }}
          />
          <span className="text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>
            {isComplete ? "3D 모델 준비됨" : isGenerating ? "생성 중..." : "대화 완료 시 3D 생성"}
          </span>
        </div>
      ...
      {showLightControls && (
        <div className="absolute top-14 right-4 z-20 p-3.5 rounded-xl border border-indigo-500/30 bg-slate-950/90 backdrop-blur-md shadow-2xl text-xs space-y-3 w-64 animate-fade-in">
